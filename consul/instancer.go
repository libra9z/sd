...
// are present.
func NewInstancer(client Client, logger l4g.Logger, service string, tags []string, passingOnly bool) *Instancer {
	s := &Instancer{
	...
	if err == nil {
		s.logger.Info("instances=%d", len(instances))
	} else {
...
func (s *Instancer) loop(lastIndex uint64) {
	...
	for {
		...
		case index == defaultIndex:
			s.logger.Error("error=index is not sane")
			time.Sleep(d)
			d = conn.Exponential(d)
		case index < lastIndex:
			s.logger.Error("error=index is less than previous; reseting to default")
